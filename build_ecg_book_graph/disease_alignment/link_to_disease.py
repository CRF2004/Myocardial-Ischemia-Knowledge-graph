"""
疾病标签对齐：PTB-XL 疾病标签 → 书籍图谱提取的 knowledge_entity

处理流程：
    加载书籍图谱提取的 JSON 数据（chapter_extraction_results.json），收集独立的 knowledge_entity（作为 candidates）。
    加载 PTB-XL 的 disease_scp_label.csv（作为 query：每条 disease label / description）。

为以下文本计算嵌入向量：
    query：PTB-XL 的 description
    candidates：书籍 knowledge_entity 文本

对每个 query：通过余弦相似度检索前 k 个书籍 knowledge_entity 候选项。
...
DISEASE_LABEL_CSV = "/mnt/data/ptb-xl-a-comprehensive-electrocardiographic-feature-dataset-1.0.1/labels/disease_scp_label.csv"
OUTPUT_JSON = "/mnt/chengrongfeng_private/SRP心肌缺血知识图谱/build_ecg_book_graph/disease_alignment/ptbxl_scp_to_book_disease.json"
...
def build_llm_prompt(query_label: str, candidates: list) -> str:
    """
    query_label: PTB-XL disease label text (e.g., description)
    candidates: list of dicts:
      { "knowledge_entity": ..., "similarity": float }
    LLM should classify each candidate into MAPS_TO / SUPERTYPE_OF / NONE
    """
    cand_lines = []
    for idx, c in enumerate(candidates, start=1):
        cand_lines.append(
            f'{idx}. knowledge_entity="{c["knowledge_entity"]}"'
        )
    cand_block = "\n".join(cand_lines)

    return f"""
    You are aligning PTB-XL disease labels (SCP-coded descriptions)
    to disease entities extracted from a textbook knowledge graph.

    The goal is STRICT SEMANTIC ALIGNMENT, not clinical reasoning.

    Given ONE query PTB-XL disease label and a ranked list of candidate textbook disease entities,
    decide the correct alignment relation for each candidate.

    Allowed relation types (choose exactly one per candidate):

    - MAPS_TO:
    The candidate textbook disease entity is semantically equivalent to the query label
    (same disease, same meaning, possible wording variation).

    - SUPERTYPE_OF:
    The candidate textbook disease entity is a more general supertype of the query label
    (query ⊂ candidate). Example: query="Inferior myocardial infarction" and candidate="Myocardial infarction".
    ...
    Do NOT use MAPS_TO for entities that merely suggest, indicate,
    support, or are risk factors for a disease.

    - SUPERTYPE_OF is mainly for clear taxonomic containment
    (query is a subtype of candidate). Do NOT infer beyond disease taxonomy.

    - If the query is a specific disease entity and the candidate is
    a broader clinical concept (ischemia, injury, etc.),
    the correct choice is NONE unless there's clear hierarchical relationship.
...
# Collect unique textbook knowledge_entity (candidates)
book_entities = sorted({r.get("knowledge_entity", "").strip() for r in book_rows if r.get("knowledge_entity")})
if not book_entities:
    raise ValueError("No knowledge_entity found in the input JSON.")

# Load PTB-XL disease labels (queries)
df = pd.read_csv(DISEASE_LABEL_CSV)

# Use description as query text
queries = df["description"].astype(str).tolist()
# First column is scp_code (column name is empty)
scp_codes = df.iloc[:, 0].astype(str).tolist()
...
# Embedding for all candidates (book knowledge_entity)
cand_embs = get_embedding(EMBED_MODEL, book_entities)
...
# =========================
# Align each query(label) -> topK book entities -> LLM decision -> output
# =========================
items = []
...
for i, query_text in tqdm(list(enumerate(queries))):
    ...
    for cand_idx in top_idx:
        ...
        candidates.append({
            ...
            "knowledge_entity": book_entities[cand_idx],
            "similarity": simv
        })
...
output = {
    "meta": {
        "task": "align_ptbxl_scp_labels_to_book_disease_entities",
        "embedding_model": EMBED_MODEL,
        "llm_model": LLM_MODEL,
        "top_k": TOP_K,
        "generated_at": generated_at,
        "input_book_graph": BOOK_GRAPH_JSON,
        "input_label_csv": DISEASE_LABEL_CSV,
...
print(f"Done. Wrote {len(items)} aligned PTB-XL disease labels to: {OUTPUT_JSON}")